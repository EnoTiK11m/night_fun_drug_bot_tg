...
- временный blacklist, пресеты, импорт/экспорт и подсказки тегов;
- избранное, галерея и экспорт изображений в ZIP;
- коллекции избранного, заметки и экспорт отдельных коллекций;
- поисковые альбомы с сортировкой, rating, типом, ориентацией и разрешением;
- пользовательская статистика и управление качеством original/sample/preview;
- постоянная компактная клавиатура Telegram для поиска, подборок, рандома, избранного и подписок;
...
- `/gallery <tags>` — показать альбом с учетом фильтров;
- `/id <post_id>` — открыть пост по ID;
- `/tags <query>` — найти подходящие теги;
- `/blacklist` — открыть настройки черного списка;
- `/blacklist add <тег>` — быстро добавить один или несколько тегов;
- `/blacklist remove <тег>` — быстро удалить один или несколько тегов;
- `/favorites` — открыть избранное;
- `/collections` — открыть коллекции избранного;
- `/stats` — статистика просмотров, поиска и избранного;
- `/whyblocked <post_id или tags>` — показать совпавшие blacklist-теги;
...
- `/health` — проверить API, SQLite, фоновые задачи и диск; только для администратора;
- `/adminstats` — runtime/DB-метрики; только для администратора;
- `/retry_failed` — повторить неудачные доставки подписок; только для администратора.
...
- `bot_features.py` — фильтры галереи, выбор качества и runtime-метрики;
